...
const logger = (0, logger_1.getLogger)('dashboard');
...
router.get('/resumo', async (req, res) => {
    try {
        ...
        // Total de empresas
        const empresasTotal = await client_1.prisma.empresa.count({ where: { ativo: true } });
        ...
        const credsOk = await client_1.prisma.credencial.findMany({
            where: { status: 'OK' },
        ...
        for (const c of certs) {
            ...
            if (diasRest != null && diasRest >= 0 && diasRest <= 30)
                certificadosVencendo++;
        }
        // Credenciais inválidas (status != OK e != NAO_TESTADO)
        const credenciaisInvalidas = await client_1.prisma.credencial.count({
            where: {
                status: { notIn: ['OK', 'NAO_TESTADO'] },
            },
        });
        // Execuções no período
        const execucoesMes = await client_1.prisma.execucao.count({
            where: { dataInicio: { gte: dataInicio } },
        });
        // Execuções com sucesso vs erro
        const execs = await client_1.prisma.execucao.findMany({
            where: { dataInicio: { gte: dataInicio } },
        ...
        // Notas encontradas
        const notas = await client_1.prisma.execucao.aggregate({
            where: { dataInicio: { gte: dataInicio } },
            _sum: {
        ...
        const notasEncontradas = (notas._sum.qtdNotasEmitidas ?? 0) + (notas._sum.qtdNotasRecebidas ?? 0);
        // Erros no mês
...
router.get('/execucoes', async (req, res) => {
    try {
        ...
        for (const e of execucoes) {
            ...
            cell.total++;
            if (e.status === 'finalizado' && !e.mensagemErro)
                cell.sucesso++;
            else
                cell.erro++;
        }
 ...
 */
router.get('/distribuicao-regime', async (req, res) => {
    try {
        const grupos = await client_1.prisma.empresa.groupBy({
            by: ['regime'],
            where: { ativo: true },
            _count: { id: true },
        });
