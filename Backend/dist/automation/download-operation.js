...
exports.shouldSkipAutoForExecution = shouldSkipAutoForExecution;
exports.markAutoSuccess = markAutoSuccess;
exports.markAutoFinalFailure = markAutoFinalFailure;
exports.clearExecutionDownloadState = clearExecutionDownloadState;
exports._resetExecutionStatesForTests = _resetExecutionStatesForTests;
exports.criarContextoOperacao = criarContextoOperacao;
exports.extrairIdentificadorDaLinha = extrairIdentificadorDaLinha;
exports.fecharModalCaptchaAtual = fecharModalCaptchaAtual;
exports.aguardarFechamentoCompletoDoModal = aguardarFechamentoCompletoDoModal;
exports.localizarNotaPorIdentificador = localizarNotaPorIdentificador;
exports.executarDownloadNotaComRetry = executarDownloadNotaComRetry;
const crypto_1 = require("crypto");
const logger_1 = require("../infrastructure/logger");
const sleep_1 = require("../utils/sleep");
const config_1 = require("../infrastructure/config");
const download_manager_1 = require("./download-manager");
const download_operation_types_1 = require("./download-operation-types");
const logger = (0, logger_1.getLogger)('download-operation');
...
function shouldSkipAutoForExecution(executionId) {
    const st = getExecutionState(executionId);
    return st.consecutiveCaptchaFailures >= config_1.CAPTCHA_CONSECUTIVE_FAILURE_LIMIT;
}
function markAutoSuccess(executionId) {
    getExecutionState(executionId).consecutiveCaptchaFailures = 0;
}
function markAutoFinalFailure(executionId) {
    getExecutionState(executionId).consecutiveCaptchaFailures += 1;
}
function clearExecutionDownloadState(executionId) {
    executionStates.delete(executionId);
}
/** Exposto para testes. */
function _resetExecutionStatesForTests() {
    executionStates.clear();
}
function atualizarOperacao(ctx, patch) {
    Object.assign(ctx, patch, { updatedAt: new Date().toISOString() });
}
function notificar(deps, stage, message, ctx) {
    if (ctx) {
        logger.info({
            ...
            chaveNfseMasked: (0, download_operation_types_1.maskNfseKey)(ctx.chaveNfse),
            stage,
        }, message);
    }
    else {
...
function criarContextoOperacao(params) {
    ...
    return {
        ...
        attempt: 1,
        maxAttempts: config_1.CAPTCHA_OPERATION_MAX_ATTEMPTS,
        status: 'pendente',
        startedAt: now,
        updatedAt: now,
 ...
 */
async function extrairIdentificadorDaLinha(rowLocator) {
...
function acharNumeroNota(textos) {
...
async function fecharModalCaptchaAtual(page, ctx, deps) {
    atualizarOperacao(ctx, { status: 'retry_reabrindo_modal' });
    if (deps) {
        notificar(deps, 'captcha_retry_closing_modal', 'Cancelando o modal atual do CAPTCHA antes do retry', ctx);
    }
    logger.info({
        operationId: ctx.operationId,
        executionId: ctx.executionId,
    ...
    const candidates = [
        ...
        {
            name: 'xpath-absoluto',
            get: () => page.locator('xpath=/html/body/div[4]/div/div/div[2]/form/div/div/div/div[2]/button[2]'),
        },
    ];
    let clicked = false;
    ...
    for (const c of candidates) {
        ...
        await button.click({ timeout: config_1.CAPTCHA_MODAL_CLOSE_TIMEOUT_MS });
        clicked = true;
        selectedSelector = c.name;
        break;
    }
    if (!clicked) {
        throw new download_operation_types_1.CaptchaModalCloseError('O botão Cancelar do modal do CAPTCHA não foi encontrado ou não estava utilizável.');
    }
    logger.info({ operationId: ctx.operationId, selector: selectedSelector }, 'Botão Cancelar do modal do CAPTCHA acionado');
    await aguardarFechamentoCompletoDoModal(page, ctx);
}
async function aguardarFechamentoCompletoDoModal(page, ctx) {
    const timeout = config_1.CAPTCHA_MODAL_CLOSE_TIMEOUT_MS;
    await page
        ...
        const stillVisible = await page
            ...
            .catch(() => false);
        if (stillVisible) {
            throw new download_operation_types_1.CaptchaModalCloseError('O botão Cancelar foi clicado, mas o modal continuou visível.');
        }
    });
    await page
    ...
    await page
        ...
        if (backdropVisible > 0) {
            throw new download_operation_types_1.CaptchaModalCloseError('O modal foi cancelado, mas o backdrop permaneceu bloqueando a página.');
        }
    });
    await page
        .locator(TABLE_ROW_SELECTOR)
        .first()
        .waitFor({ state: 'visible', timeout: config_1.CAPTCHA_NOTE_RELOCATION_TIMEOUT_MS })
        .catch(() => undefined);
    logger.info({
        operationId: ctx.operationId,
        modalClosed: true,
...
async function linhaCorresponde(row, chaveNfse) {
    try {
...
async function localizarNotaPorIdentificador(page, ctx) {
    const deadline = Date.now() + config_1.CAPTCHA_NOTE_RELOCATION_TIMEOUT_MS;
    const tentarNaPaginaAtual = async () => {
        await page.waitForSelector(TABLE_ROW_SELECTOR, {
            timeout: Math.min(10000, config_1.CAPTCHA_NOTE_RELOCATION_TIMEOUT_MS),
        }).catch(() => undefined);
        const rows = page.locator(TABLE_ROW_SELECTOR);
        const total = await rows.count();
        if (ctx.indiceLinhaOriginal != null &&
            ctx.indiceLinhaOriginal >= 0 &&
            ctx.indiceLinhaOriginal < total) {
            const tip = rows.nth(ctx.indiceLinhaOriginal);
            if (await linhaCorresponde(tip, ctx.chaveNfse))
                return tip;
        }
    ...
    throw new download_operation_types_1.NotaNaoEncontradaParaRetryError(`Nota nao encontrada para retry: ${(0, download_operation_types_1.maskNfseKey)(ctx.chaveNfse)} (${ctx.tipoArquivo})`);
}
...
async function abrirMenuEObterLink(page, row, ctx) {
...
async function tentarResolverCaptchaSeNecessario(page, downloadPromise, ctx, deps, forceManual) {
    const detectTimeout = Math.max(CAPTCHA_DETECT_TIMEOUT_MS, config_1.CAPTCHA_NEW_CHALLENGE_TIMEOUT_MS);
    ...
    if (!skipAuto && (modo === 'auto' || modo === 'auto_manual')) {
        atualizarOperacao(ctx, { status: 'captcha_resolvendo' });
        notificar(deps, ctx.attempt > 1 ? 'captcha_retry_resolving' : 'captcha_resolvendo', ctx.attempt > 1
            ? `Criando nova tarefa 2Captcha para desafio regenerado (tentativa ${ctx.attempt}/${ctx.maxAttempts})`
            : 'Resolvendo hCaptcha via 2captcha…', ctx);
    ...
    notificar(deps, 'captcha_fallback_manual', `Aguardando resolução MANUAL para ${ctx.tipoArquivo.toUpperCase()} desta nota`, ctx);
    await deps.aguardarResolucaoManual(page);
}
async function executarTentativaDeDownload(page, row, ctx, deps, forceManual) {
    atualizarOperacao(ctx, { status: 'abrindo_download' });
    const link = await abrirMenuEObterLink(page, row, ctx);
    const downloadTimeout = Math.max(config_1.CAPTCHA_SOLVE_TIMEOUT_MS, config_1.CAPTCHA_MANUAL_TIMEOUT_MS, DOWNLOAD_TIMEOUT_MS) +
    ...
    const savedPath = await (0, download_manager_1.salvarDownloadDireto)(download, ctx.basePath, ctx.nomeContabilidade, ctx.mesExecucaoExtenso, ctx.nomeEmpresa, ctx.tipoNota, ctx.nomeArquivoPrefixo);
    const validation = await (0, download_manager_1.validarArquivoBaixado)(savedPath, ctx.tipoArquivo);
    if (!validation.valid) {
        await (0, download_manager_1.removerArquivoInvalido)(savedPath);
        throw new Error(`Arquivo ${ctx.tipoArquivo} inválido após download: ${validation.reason || 'desconhecido'}`);
    }
    return savedPath;
}
async function prepararNovaTentativa(page, ctx, deps) {
    atualizarOperacao(ctx, { status: 'retry_pendente' });
    notificar(deps, 'captcha_retry_preparing', `CAPTCHA não solucionado. Preparando nova tentativa automática ${ctx.attempt + 1}/${ctx.maxAttempts} para o ${ctx.tipoArquivo.toUpperCase()} da nota.`, ctx);
    ...
    await (0, sleep_1.sleep)(config_1.CAPTCHA_OPERATION_RETRY_DELAY_MS);
    notificar(deps, 'captcha_retry_relocating_note', 'Fechando o desafio anterior e localizando novamente a nota.', ctx);
    await page
        .locator(TABLE_ROW_SELECTOR)
        .first()
        .waitFor({ state: 'visible', timeout: config_1.CAPTCHA_NOTE_RELOCATION_TIMEOUT_MS });
 ...
 */
async function executarDownloadNotaComRetry(page, rowInicial, ctx, deps) {
    getExecutionState(ctx.executionId).currentOperationId = ctx.operationId;
    const existing = await (0, download_manager_1.localizarArquivoExistenteValido)(ctx.basePath, ctx.nomeContabilidade, ctx.mesExecucaoExtenso, ctx.nomeEmpresa, ctx.tipoNota, ctx.tipoArquivo, ctx.nomeArquivoPrefixo);
    if (existing.valid && existing.path) {
        ...
        notificar(deps, 'download_completed', `Arquivo ${ctx.tipoArquivo.toUpperCase()} já existia e foi validado — pulando download`, ctx);
    ...
    for (let attempt = ctx.attempt; attempt <= ctx.maxAttempts; attempt++) {
        ...
        try {
            if (attempt > 1) {
                ...
                notificar(deps, 'captcha_retry_opening_new_challenge', 'Nota localizada. Abrindo um novo desafio hCaptcha.', ctx);
            ...
            const pathSaved = await executarTentativaDeDownload(page, row, ctx, deps, false);
            atualizarOperacao(ctx, {
                status: 'concluido',
            ...
            notificar(deps, 'download_completed', `Download ${ctx.tipoArquivo.toUpperCase()} concluído`, ctx);
            return { success: true, skipped: false, path: pathSaved };
        }
        catch (error) {
            lastError = error;
            const classification = (0, download_operation_types_1.classificarErroDaOperacao)(error);
            atualizarOperacao(ctx, {
    ...
    const classification = (0, download_operation_types_1.classificarErroDaOperacao)(lastError);
    markAutoFinalFailure(ctx.executionId);
    if (config_1.CAPTCHA_OPERATION_FALLBACK_MANUAL &&
        classification.action !== 'FAIL_CONFIGURATION' &&
        config_1.CAPTCHA_MODE !== 'auto') {
        notificar(deps, 'captcha_retry_exhausted', `Tentativas automáticas esgotadas (${ctx.maxAttempts}). Aguardando resolução manual para o ${ctx.tipoArquivo.toUpperCase()} desta nota.`, ctx);
        try {
            ...
            if (!modalVisivel) {
                row = await localizarNotaPorIdentificador(page, ctx);
                const pathSaved = await executarTentativaDeDownload(page, row, ctx, deps, true);
                atualizarOperacao(ctx, {
                    status: 'concluido',
                    outputPath: pathSaved,
                    fileValidated: true,
            ...
            atualizarOperacao(ctx, { status: 'fallback_manual' });
            await deps.aguardarResolucaoManual(page);
            const downloadTimeout = Math.max(config_1.CAPTCHA_MANUAL_TIMEOUT_MS, DOWNLOAD_TIMEOUT_MS) + CAPTCHA_DOWNLOAD_BUFFER_MS;
            const download = await page.waitForEvent('download', { timeout: downloadTimeout });
            const pathSaved = await (0, download_manager_1.salvarDownloadDireto)(download, ctx.basePath, ctx.nomeContabilidade, ctx.mesExecucaoExtenso, ctx.nomeEmpresa, ctx.tipoNota, ctx.nomeArquivoPrefixo);
            const validation = await (0, download_manager_1.validarArquivoBaixado)(pathSaved, ctx.tipoArquivo);
            if (!validation.valid) {
                await (0, download_manager_1.removerArquivoInvalido)(pathSaved);
            ...
            atualizarOperacao(ctx, {
                status: 'falhou',
                lastErrorMessage: manualErr.message.slice(0, 500),
            });
            return {
                success: false,
    ...
    atualizarOperacao(ctx, { status: 'falhou' });
    return {
        success: false,
