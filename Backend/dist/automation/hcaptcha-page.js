...
exports.CAPTCHA_SUBMIT_SELECTOR = void 0;
exports.capturarDadosCaptcha = capturarDadosCaptcha;
exports.injetarTokenHCaptcha = injetarTokenHCaptcha;
exports.observarResultadoPortalAposSubmit = observarResultadoPortalAposSubmit;
exports.aplicarTokenCaptchaNaPagina = aplicarTokenCaptchaNaPagina;
const config_1 = require("../infrastructure/config");
const logger_1 = require("../infrastructure/logger");
const captcha_diagnostic_1 = require("./captcha-diagnostic");
const logger = (0, logger_1.getLogger)('hcaptcha-page');
exports.CAPTCHA_SUBMIT_SELECTOR = '#btnSubmitHCaptcha';
...
async function tentarExtrairAtributosWidget(page) {
    const script = `(() => {
 ...
 */
async function injetarTokenHCaptcha(page, token, options) {
 ...
 */
async function observarResultadoPortalAposSubmit(page, submitAction, observeMs = 8000) {
    const relevantUrls = [];
    ...
    const onRequest = (req) => {
        const url = req.url();
        if (/hcaptcha|captcha|nfse|download|validar|confirm/i.test(url) &&
    ...
    const modalVisibleAfter = await page
        .locator(exports.CAPTCHA_SUBMIT_SELECTOR)
        .isVisible()
        .catch(() => false);
    let result = 'UNKNOWN';
    if (rejectedHint)
        result = 'REJECTED';
    else if (newChallengeHint)
        result = 'NEW_CHALLENGE_CREATED';
    else if (!modalVisibleAfter && requestSent)
        result = 'ACCEPTED';
    else if (!modalVisibleAfter && !requestSent)
        result = 'ACCEPTED'; // download pode ter sido local
    else if (modalVisibleAfter && !requestSent)
        result = 'NO_REQUEST_SENT';
    else if (modalVisibleAfter && requestSent)
        result = 'MODAL_REMAINED_OPEN';
    return {
        result,
        requestSent,
        responseStatus,
 ...
 */
async function aplicarTokenCaptchaNaPagina(page, token, options) {
    const injection = await injetarTokenHCaptcha(page, token, {
        callbackName: options?.callbackName,
    });
    const observe = options?.observePortal ?? (0, captcha_diagnostic_1.isCaptchaDebug)();
    if (!observe) {
        const botaoConfirmar = page.locator(exports.CAPTCHA_SUBMIT_SELECTOR);
...
//# sourceMappingURL=hcaptcha-page.js.map